...
import { MailOutlined, LockOutlined, UserOutlined, PhoneOutlined, GiftOutlined, TrophyOutlined, ArrowRightOutlined, CheckCircleFilled } from '@ant-design/icons'
import { useAuth } from '../context/AuthContext'

const CONFIG = {
    subtitle: 'Trainer Signup',
    heroTitle: 'Run your own coaching business.',
    heroSub: 'Sign up, verify your email, pick a plan, and start coaching your own clients — no affiliation needed.',
    highlights: [
        'Choose the plan that fits your client list',
...
// Step 1 of Trainer self-signup: Signup -> OTP -> Plan -> Payment -> Pending Approval.
export default function Signup() {
    ...
    const submit = async () => {
        ...
        try {
            ...
            if (data.referralWarning) message.warning(`Referral code not applied: ${data.referralWarning}`, 8)
        ...
        <div className="grid min-h-screen lg:grid-cols-[1.05fr_1fr]">
            ...
            <div className="auth-panel flex items-center justify-center p-6 sm:p-10">
                <div className="w-full max-w-sm">
                    ...
                    <h2 className="text-2xl font-extrabold tracking-tight text-text-primary">Create your Trainer account</h2>
                    <p className="mt-1.5 text-sm text-text-secondary">Coach your own clients on FitTrack with a plan that suits you.</p>
                    ...
                    <Form form={form} layout="vertical" requiredMark={false} className="mt-8" onFinish={submit} initialValues={{ referralCode: initialRef }}>
                        <Form.Item name="name" label="Full name" rules={[{ required: true, message: 'Enter your name' }]}>
                            <Input size="large" prefix={<UserOutlined />} placeholder="e.g. Marcus Bennett" />
                        </Form.Item>
                        <Form.Item
                            name="email"
                            label="Email"
                        ...
                        <Form.Item name="specialization" label="Specialization (optional)">
                            <Input size="large" prefix={<TrophyOutlined />} placeholder="e.g. Strength & Conditioning" />
                        </Form.Item>
                        ...
                        <Form.Item name="referralCode" label="Referral code (optional)">
                            <Input size="large" prefix={<GiftOutlined />} placeholder="e.g. MARCUS-7K3M" style={{ textTransform: 'uppercase' }} />
                        </Form.Item>

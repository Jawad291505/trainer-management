...
| `exerciseTechniques.json` | Training-technique enum every exercise carries via its `technique` key (`standard` \| `tut` \| `superset`). Each item: `{ key, label, description }`. Consumed by admin, trainer and client apps for the Add/Edit Exercise forms and the technique badge shown wherever exercises are displayed. `standard` is the implicit default for any exercise missing the field. | `../WhatsApp Image 2026-09-01 at 12.16.52*.jpeg` (workout-routine reference sheet: TUT + Super Set notes) |

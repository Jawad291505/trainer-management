...
// GET /api/trainers?status=&search=   (admin)
export const listTrainers = asyncHandler(async (req, res) => {
    const userMatch = {}
    if (req.query.search) userMatch.name = { $regex: String(req.query.search).trim(), $options: 'i' }

    let trainers = await Trainer.find(req.query.status ? { status: req.query.status } : {})
...
// GET /api/trainers/:id   (admin, or the trainer themselves)
export const getTrainer = asyncHandler(async (req, res) => {
    const id = req.params.id === 'me' ? req.trainer?._id : req.params.id
    const trainer = await Trainer.findById(id).populate('user', 'name email avatarColor status phone')
...
// POST /api/trainers   (admin) — provision a trainer account
export const createTrainer = asyncHandler(async (req, res) => {
    const { name, email, password, specialization, capacity } = req.body
    ...
    res.status(201).json(flatten(trainer))
})

// PATCH /api/trainers/:id   (admin) — profile, status, capacity, rating
...
// PATCH /api/trainers/:id/capacity   (admin)  Body: { delta } or { capacity }
...
// DELETE /api/trainers/:id   (admin) — also unassigns their clients
